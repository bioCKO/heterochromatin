...
myColors<-c(rep("gray30",1),rep("firebrick1",4),rep("dodgerblue",1),rep("sandybrown",2),rep("gold",2))

#boxplot(repVariability,names=paste(uniq_names,lengths(repVariability),"sd:",sd),las=2,col=myColors,outline=TRUE)

len <- sapply(repVariability,length)
n <- max( len )
len <- n - len
dt<-mapply( function(x,y) c( x , rep( NA , y ) ) , repVariability , len, SIMPLIFY = TRUE)
rownames(dt)<-NULL
...
myColors<-c("black","coral","skyblue2","darkorange","gold")
...
plot(read_counts,abundances,main="No underlying bias in relationship between read counts and abundances",sub=paste("correlation coefficient:",cor(read_counts,abundances)))
...
for (ape in unique(colnames(explData))) {
  ...
  if (!is.null(n) && n>1) {
    ...
    #NOTE: SORTED BY BOTH ROWSUMS (repeat abundances) and COLSUMS (individual abundances)
    t<-head(t,n=10)
    print(t)
    ...
    for (c in colnames(t)) {
      ...
      text(1:ncol(indivSorted),nrow(indivSorted)/2,labels=colnames(indivSorted),las=1,srt=90,col="darkgreen",cex=0.75)
    }
