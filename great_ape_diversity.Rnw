...
par(mfrow=c(1,2),cex=1.1,oma = c(5,4,0,0) + 0.1, mar = c(0,0,2.2,2.2) + 0.1)
...
colnames(top)<-paste(lapply(strsplit(colnames(top),"\\|"), function(x) paste(x[3],x[2])))
...
hc <- hclust(distSamples,method="ward.D") #method="ward.D", "ward.D2", "single", "complete", "average" (= UPGMA), "mcquitty" (= WPGMA), "median" (= WPGMC) or "centroid" (= UPGMC).
...
library(rgl)
par(mfrow=c(2,1))
pca_data<-as.data.frame(na.omit(sample[,1:3]))
