...
#dist<-dist((sampleToCluster),method="euclidean")

m1 <- as.matrix(dist)
m1[upper.tri(m1)] <- 0
library(reshape2)
m2 <- subset(melt(m1), value!=0)
m2$X1<-gsub(pattern='(.*)[.](.*)','\\1',m2$X1)
m2$X2<-gsub(pattern='(.*)[.](.*)','\\1',m2$X2)
m2<-m2[!m2$X1==m2$X2,] #remove same species comparisons
m2<-(m2[order(m2$value),3:1])


distOriginal=as.dist(1-cor(new_dfr))

hc <- hclust(dist,method="complete") #method="ward.D", "ward.D2", "single", "complete", "average" (= UPGMA), "mcquitty" (= WPGMA), "median" (= WPGMC) or "centroid" (= UPGMC).
hcSingle <- hclust(dist,method="single") #method="ward.D", "ward.D2", "single", "complete", "average" (= UPGMA), "mcquitty" (= WPGMA), "median" (= WPGMC) or "centroid" (= UPGMC).
...
names_and_sexes<-paste(lapply(strsplit(rownames(sample)," "), function(x) x[1]),lapply(strsplit(rownames(sample)," "), function(x) x[3]))
...
plot(dend, main = "dend")
plot(dendSingle, main = "dendSingle")
#hexplom(sample)
dendrograms <- dendlist(dend, dendSingle)
#tanglegram(dendrograms,common_subtrees_color_branches = TRUE)
...
#colSums(non_zero[grepl(x,rownames(non_zero))==TRUE,])!=0
repVariability<-lapply(uniq_names, function(x) rowMeans(non_zero[grepl(x,rownames(non_zero))==TRUE,colSums(non_zero[grepl(x,rownames(non_zero))==TRUE,])!=0],na.rm = TRUE))
...
#dt<-as.data.frame(dt)
dotchart(dt,labels="",gcol=myColors,lcolor=rep(myColors,each=n),xlab="repeat counts per 1 million reads",pch=20,col="gray30")


uniq_names<-c("Homo","Pan_troglodytes","Pan_paniscus","Gorilla","Pongo")
myColors<-c("gray30","darkgreen","dodgerblue","firebrick1","gold")

repVariabilityBig<-lapply(uniq_names, function(x) rowMeans(non_zero[grepl(x,rownames(non_zero))==TRUE,colSums(non_zero[grepl(x,rownames(non_zero))==TRUE,])!=0],na.rm = TRUE))
boxplot(repVariabilityBig,names=paste(uniq_names,lengths(repVariabilityBig)),las=2,col=myColors,outline=TRUE)

@

<<controls>>=
read_counts<-getFilteredReadCounts(colnames(abundant))[2:ncol(abundant)]
abundances<-apply(abundant[,2:ncol(abundant)],2,mean)
plot(read_counts,abundances,main="Number of reads in a sample is independent of the repeat abundance in a sample",sub=paste("correlation coefficient:",cor(read_counts,abundances)),cex.lab=1.2)
...
plot(table(sort(nchar(as.character(abundant$unit)))),xlab="unit size",ylab="number of times an unit size is present",main="694 distinct repeated motifs and their repeat unit sizes")
@
...
palette(c("blue","darkgreen","red","black","orange"))
...
#species<-paste(lapply(strsplit(rownames(sample)," "), function(x) x[2]))
#sample$sex<-sex
pca_data_sex<-as.data.frame(sample) #SUBSAMPLE ONLY TO CHIMPANZEES #grepl("Gorilla|Pan_troglodytes",sample$species)
pca_data_sex$sex<-sex
rownames(pca_data_sex)<- make.names(pca_data_sex$sex, unique=TRUE)
pca_data_sex<-pca_data_sex[1:80,c(1:10,ncol(pca_data_sex)-1)] #subsample
...
plot3d(pc_sex$scores[,4:6],col=as.numeric(factor(sex))+1)
plot.new()
...
for (ape in unique(colnames(explData))) {
  ...
  if (!is.null(n) && n>1) {
    ...
    #apply(t, 1, function(x) paste(names(x)[order(x)], collapse = " "))
    ...
    for (c in colnames(t)) {
      ...
      plot(indivOrder,col="blue",main=c,sub=ape,ylim=c(0,nrow(indivSorted)),ylab="individual order based on particular repeat")
      abline(h=median(indivOrder),col="red")
      text(1:ncol(indivSorted),nrow(indivSorted)/2,labels=names(indivOrder),las=1,srt=90,col="darkgreen",cex=0.75)
    }
...
heatmap.2(binary,dendrogram="none", Colv="Rowv",trace='none',col=mycol,cexRow=0.1,cexCol=0.75)
#colnames(binary)<-sampleToCluster[,1]
#binary<-binary[-1,]
binary<-as.data.frame(binary)
