...
SRR_readC_afterF = read.table(("SRR_readC_afterF"),header=FALSE)
layout = read.table("full_layout", header=TRUE, sep = ",")
layout<-layout[layout$Run %in% SRR_readC_afterF$V1,] #keep only those that have successful filtering information
...
getFilteredReadCounts <- function(files) {
  SRR_ids<-gsub( "\\|.*$", "", files)
  merged<-merge(SRR_ids,SRR_readC_afterF,by=1)
...
tables <- lapply(files, function(x) read.table(gsub( "\\|", "_", x), nrows=10, col.names=c("unit", gsub("_1.fastq.counts.sortedFilt", "", x))))
...
dropShortReads <-function(files,read_length_threshold) { 
  ...
  SRR_ids_to_exclude<-gsub( "_.*$", "", SRR_read_lengths[SRR_read_lengths$V2<read_length_threshold,]$V1)
...
SRR_read_lengths = read.table("SRR_read_lengths", header=FALSE)
SRR_read_lengths<-SRR_read_lengths[grep("_1.fastq",SRR_read_lengths$V1),] #keep only forward reads
...
getCandidatesForSpecies <- function(row,species,threshold) {
    ...
    return(c(as.character(row$unit),as.numeric(pvalue),round(mean_cumulative_length)))
    }
}
...
plotFigure <- function(toPlot) {
  ...
  plot(rep(0,dim(toPlotAggregated)[1])+j,toPlotAggregated$counts,col=factor(toPlotAggregated$fullSpecies),main=unit,pch=symb,ylab="repeat counts normalized to 1 million reads",cex.main=0.8,xlim=c(0,2),xaxt='n',xlab="",cex=1.8)
  ...
  my.vioplot(getFemales(toPlotAggregated,"Chimpanzee"),getMales(toPlotAggregated,"Chimpanzee"),getFemales(toPlotAggregated,"Bonobo"),getMales(toPlotAggregated,"Bonobo"),getFemales(toPlotAggregated,"Gorilla"),getMales(toPlotAggregated,"Gorilla"),getFemales(toPlotAggregated,"Orangutan"),getMales(toPlotAggregated,"Orangutan"),col=c("tomato","skyblue","tomato","skyblue","tomato","skyblue","tomato","skyblue"),names=c("Chimpanzee","Chimpanzee","Bonobo","Bonobo","Gorilla","Gorilla","Orangutan","Orangutan"))
  ...
  boxplot(
    ...
    col=c("tomato","skyblue","mediumseagreen","tomato","skyblue","mediumseagreen","tomato","skyblue","mediumseagreen","tomato","skyblue","mediumseagreen"),names=c("C","C","P","B","B","P","G","G","P","O","O","P"))
...
<<trf_vs_phobos>>=
trf<-read.table("/Users/alice/Desktop/projects/heterochromatin/great_ape_diversity/trf_vs_phobos/orangutan_sample.fasta.trf_unitL_totalL_25", header=FALSE, sep = "\t")
phobos<-read.table("/Users/alice/Desktop/projects/heterochromatin/great_ape_diversity/trf_vs_phobos/orangutan_sample.fasta.out_unitL_totalL_25", header=FALSE, sep = "\t")
