...
head(cumstat) #candidates for FISH
...
getCandidatesForSpecies <- function(row,species,threshold) {
    ...
    #female_counts<-nrow(toPlotAggregated[toPlotAggregated$sex=="F",])
...
Candidates_Human<-getTopCandidatesForSpecies("Human",threshold)
...
plotFigure <- function(toPlot) {
  ...
  females_pch<-gsub("Gorilla","17", females_pch)
  females_pch<-gsub("Bonobo","15", females_pch)
  females_pch<-gsub("Chimpanzee","16", females_pch)
  females_pch<-gsub("Orangutan","18", females_pch)
  females_pch<-gsub("Homo","14", females_pch)
  females_pch<-as.numeric(females_pch)
  
  females_col_leg<-females$fullSpecies
  females_col_leg<-gsub("Gorilla_beringei_graueri","#d7191c", females_col_leg)
  females_col_leg<-gsub("Gorilla_gorilla_diehli","orange", females_col_leg)
  females_col_leg<-gsub("Gorilla_gorilla_gorilla","gold", females_col_leg)
  females_col_leg<-gsub("Pan_paniscus","#2c7bb6", females_col_leg)
  females_col_leg<-gsub("Pan_troglodytes_ellioti","#41b6c4", females_col_leg)
  females_col_leg<-gsub("Pan_troglodytes_schweinfurthii","#c2e699", females_col_leg)
  females_col_leg<-gsub("Pan_troglodytes_troglodytes","#31a354", females_col_leg)
  females_col_leg<-gsub("Pan_troglodytes_verus","#006837", females_col_leg)
  females_col_leg<-gsub("Pan_troglodytes_verus_x_troglodytes","cyan2", females_col_leg)
  females_col_leg<-gsub("Pongo_abelii","#c994c7", females_col_leg)
  females_col_leg<-gsub("Pongo_pygmaeus","#dd1c77", females_col_leg)
  females_col_leg<-gsub("Homo_sapiens","black", females_col_leg)
  
  males_col_leg<-males$fullSpecies
  males_col_leg<-gsub("Gorilla_beringei_graueri","#d7191c", males_col_leg)
  males_col_leg<-gsub("Gorilla_gorilla_diehli","orange", males_col_leg)
  males_col_leg<-gsub("Gorilla_gorilla_gorilla","gold", males_col_leg)
  males_col_leg<-gsub("Pan_paniscus","#2c7bb6", males_col_leg)
  males_col_leg<-gsub("Pan_troglodytes_ellioti","#41b6c4", males_col_leg)
  males_col_leg<-gsub("Pan_troglodytes_schweinfurthii","#c2e699", males_col_leg)
  males_col_leg<-gsub("Pan_troglodytes_troglodytes","#31a354", males_col_leg)
  males_col_leg<-gsub("Pan_troglodytes_verus_x_troglodytes","cyan2", males_col_leg)
  males_col_leg<-gsub("Pan_troglodytes_verus","#006837", males_col_leg)
  males_col_leg<-gsub("Pongo_abelii","#c994c7", males_col_leg)
  males_col_leg<-gsub("Pongo_pygmaeus","#dd1c77", males_col_leg)
  males_col_leg<-gsub("Homo_sapiens","black", males_col_leg)
  
  males_pch<-males$species
  males_pch<-gsub("Gorilla","17", males_pch)
  males_pch<-gsub("Bonobo","15", males_pch)
  males_pch<-gsub("Chimpanzee","16", males_pch)
  males_pch<-gsub("Orangutan","18", males_pch)
  males_pch<-gsub("Homo","14", males_pch)
  ...
  legend(x="topleft", legend = levels(factor(toPlotAggregated$fullSpecies)), col = col.list, pch=as.numeric(c("17","17","17","15","16","16","16","16","16","18","18")), cex=0.6,bty = "n", y.intersp=2,xpd=TRUE)
  
  my.vioplot(getFemales(toPlotAggregated,"Chimpanzee"),getMales(toPlotAggregated,"Chimpanzee"),getFemales(toPlotAggregated,"Bonobo"),getMales(toPlotAggregated,"Bonobo"),getFemales(toPlotAggregated,"Gorilla"),getMales(toPlotAggregated,"Gorilla"),getFemales(toPlotAggregated,"Orangutan"),getMales(toPlotAggregated,"Orangutan"),col=c("tomato","skyblue","tomato","skyblue","tomato","skyblue","tomato","skyblue"),names=c("Chimpanzee","Chimpanzee","Bonobo","Bonobo","Gorilla","Gorilla","Orangutan","Orangutan"),ylim=c(0,max(toPlotAggregated$counts)))
  ...
  mtext(as.character(round(tChimpanzee$p.value,2)),col=ifelse(tChimpanzee$p.value<0.05,"red","black"),at=2)
  mtext(as.character(round(tBonobo$p.value,2)),col=ifelse(tBonobo$p.value<0.05,"red","black"),at=4)
  mtext(as.character(round(tGorilla$p.value,2)),col=ifelse(tGorilla$p.value<0.05,"red","black"),at=6)
  mtext(as.character(round(tOrangutan$p.value,2)),col=ifelse(tOrangutan$p.value<0.05,"red","black"),at=8)
...
par(mfrow=c(1,3),cex=1.1,oma = c(5,4,0,0) + 0.1, mar = c(0,0,2.2,2.2) + 0.1)
for (unit in unitList) {
  ...
  if (ncol(toPlot)>0) { #the unit does exist
    ...
    print(unit)
    col.list <- c("#d7191c","orange","gold","#2c7bb6","#41b6c4","#c2e699","#31a354","#006837","cyan2","#c994c7","#dd1c77")
    palette(col.list)
    plotFigure(toPlot)
  }
}
...
top<-abundant[1:2137171,]

sample = top[,2:ncol(top)]

#sample<-sample[,grepl("\\|F",colnames(sample))==TRUE] #PICK ONLY FEMALES
sample<-t(sample)
...
groupCodes <-l
rownames(sample)<-l
colorCodes <- c(Bonobo="blue", Chimpanzee="darkgreen", Gorilla="red", Orangutan="orange")
