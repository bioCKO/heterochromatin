...
plotFile <- function(file) {
  ...
  return(c(pvaluesForMotif,slopesForMotif))
...
heatmap.2(toPlot,dendrogram='row',Rowv=TRUE,Colv=FALSE,col=my_palette,trace="row",tracecol="gray",lmat=lmat,lwid=lwid,margins = c(6.5, 6),key.title ="p-values",cellnote=annotation,notecol="black")
