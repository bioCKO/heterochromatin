...
calculateIPDforWindow <- function(chromosome,start,end,numberOfPasses) { 
  ...
  if (length(subreads)==numberOfPasses) {
    #print(length(subreads))
    print(paste(chromosome,start,end))
    for (pass in subreads) {
...
by(coordinates, 1:nrow(coordinates), function(row) calculateIPDforWindow(paste0("chr",row[1]),row[2],row[3],6))
