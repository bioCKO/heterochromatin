...
calculateIPDforWindow <- function(chromosome,start,end,numberOfPasses) { 
  ...
  for (mol in split(molecules, by = c("moleculeID"))) { 
    ...
    if (subreads_in_mol==numberOfPasses) {
      ...
      #print(chunk$strand)
      ideal_strandness<-rep(c(0,1),length(chunk$strand)/2)
      if(all.equal(chunk$strand,ideal_strandness)==TRUE) { #strands interleave as expected
        ...
        for (i in chunk[order(chunk$rStart)]$idx) {
          ...
          number_of_positions_with_IPDs<-dim(idx_chunk)[1]
          print(number_of_positions_with_IPDs)
          
          if (number_of_positions_with_IPDs>0) {
          ...
          filename<-paste0(paste(chromosome,start,end,sep="-"), "_", m, ".IPDs.txt")
          if (file.exists(filename)) file.remove(filename) #remove file if it exists
...
calculateIPDforWindow("chr21",9426500,9426599,4)
