...
for X in "${array[@]}"; 
	do echo $X; 
		...
		echo ${bam}
		bedtools coverage -a $1 -b $bam -d >${X}.${motif_file}.depth
		bedtools groupby -i ${X}.${motif_file}.depth -g 1,4,5 -c 11 -o collapse | awk '{print $1 "\t" $2 "\t" $3 "\t" ($3-$2+1) "\t" $4}' | sed s'/,/\t/g' >${X}.${motif_file}.depth.summary
	done;
