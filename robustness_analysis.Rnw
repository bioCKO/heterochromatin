...
options(scipen=10)
length_variable<-140000 
repeat_distribution<-rexp(n=10000, rate=1/20000) #rate=1/5000 for length_variable<-35000
...
hist(Rvector,xlab="simulated read length",ylab="counts",main="Generated length of repeats",col="gold",sub=paste("n: ",length(Rvector)),breaks=seq(0,200000,1000),xlim=c(0,50000))
...
for (file in (filenames)) {
  tryCatch({
    if (file.size(file) > 0){
      ...
      hist(de_novo$V1,xlab="simulated read length",ylab="counts",main=NULL,col=ifelse(grepl("14000",file)==TRUE,'green','orange'),sub=paste("n: ",length(de_novo$V1)),breaks=seq(0,200000,1000),xlim=c(0,50000))
    ...
    print(paste(file,"Read.table didn't work!:  ",err))
  })
}
