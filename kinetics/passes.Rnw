...
calculateIPDforWindow <-
  function(chromosome, start, end, numberOfPasses) {
    ...
    if (length(unique(sort(molecules$moleculeID)))>0) {
      ...
      for (molID in unique(sort(molecules$moleculeID))) {
...
setwd("/Volumes/Przeworski_baboon_data/kinetics_cmp")
...
for (myChromosome in c(seq(22,8,-1),seq(6,1,-1))) { #c(seq(22,8,-1))
  print(myChromosome)
  cmpH5candidate<-paste0("chr",myChromosome,"_P6.cmp.h5")
  
  if (file.exists(cmpH5candidate)) { #does cmp file exists?
    
    passesStatCandidate<-paste0(myChromosome,".GQuadPlusFeatureOnly.mf.gff.intersect")
    ...
    if (file.exists(passesStatCandidate)) { #does passesStat file exists?
      ...
      by(coordinates, 1:nrow(coordinates), function(row) calculateIPDforWindow(paste0("chr", row[1]), row[2], row[3], 5))
    }
    else {
...
save(list=c("IPDlistPASS1_0", "IPDlistPASS1_1", "IPDlistPASS2_0", "IPDlistPASS2_1","IPDlistPASS3_0", "IPDlistPASS3_1","IPDlistPASS4_0", "IPDlistPASS4_1","IPDlistPASS5_0", "IPDlistPASS5_1"), file="IPDlist_passes.rda")
