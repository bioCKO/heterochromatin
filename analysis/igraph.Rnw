...
<<>>=
library(reshape)
m <- read.table("/Users/polly/Desktop/projects/heterochromatin/homology/network_files/Distance_data_from_megaOctober11",sep="\t",fill=TRUE,header=TRUE,row.names = 1)
m<-as.matrix(m)
print(dim(m))
...
V(net)$size <- range01(densities) * 30 + 12
