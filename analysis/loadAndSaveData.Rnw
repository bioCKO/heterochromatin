...
data_directory<-"/Users/alice/Desktop/projects/heterochromatin/refactored/" #"/Users/alice/Desktop/projects/heterochromatin/great_ape_diversity/counts75/"
...
<<FREQUENCY>>=
d<-read.table("/Users/alice/Desktop/projects/heterochromatin/refactored/big.table.with.header.1.fastq.dat_Header.txt.rawcounts.sortedFilt.txt",sep=" ",header=TRUE,row.names=1,na.strings=NA,fill=TRUE)
...
<<DENSITY>>=
d<-read.table("/Users/alice/Desktop/projects/heterochromatin/refactored/big.table.with.header.1.fastq.dat_Header.txt.rawlengths.sortedFilt.txt",sep=" ",header=TRUE,row.names=1,na.strings=NA,fill=TRUE)
d<-as.data.frame(d) #convert to dataframe
d[is.na(d)] <- 0
...
#DENSITY
density<-data[,columns_to_select] #RESTRICT ONLY TO FILTERED FILES
density<-density[order(rowSums(density),decreasing=T),]
density<-density[nchar(as.character(rownames(density)))<=50,] #remove handful of repeat motifs with unit sizes over 50
