...
d<-read.table("/Users/alice/Desktop/projects/heterochromatin/refactored/big.table.with.header.rawcounts.sortedFilt.txt",sep=" ",header=TRUE,row.names=1,na.strings=NA,fill=TRUE)
...
<<DENSITY>>=
d<-read.table("/Users/alice/Desktop/projects/heterochromatin/refactored/big.table.with.header.rawlengths.sortedFilt.txt",sep=" ",header=TRUE,row.names=1,na.strings=NA,fill=TRUE)
d<-as.data.frame(d) #convert to dataframe
d[is.na(d)] <- 0
