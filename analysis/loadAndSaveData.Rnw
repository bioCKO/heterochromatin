...
merged.data.frame = Reduce(function(...) merge(..., all=T), dt_list)

library(dplyr)

dt_list %>%
    Reduce(function(dtf1,dtf2) full_join(dtf1,dtf2,by="unit"), .)

#MERGE ALL DATASETS INTO SINGLE TABLE
mydt <- function(...) base::merge(..., by="unit", all=TRUE) #View(base::merge(dt_list[1],dt_list[221],by="unit", all=TRUE))
data <- Reduce(mydt, dt_list)
data[data$unit=="C",c(1,221)]

Reduce(function(dtf1, dtf2) merge(dtf1, dtf2, by = "unit", all = TRUE),dt_list)
...
<<>>=
for(i in seq(from=1, to=370, by=1)){
  print(i)
  print(table(as.data.frame(tables[i])$unit=="C"))
}
@
...
abundant<-data[,columns_to_select]
#abundant<-head(abundant,10000)
@
