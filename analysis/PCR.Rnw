...
counts<-as.data.frame(cbind(94110593,3587652,28278372))
...
plot(PCRcounts$PCRplus1,PCRcounts$PCRplus2,main="counts per 1 million reads",col="red")
abline(a=c(0,0),b=(1),col="red")

plot(PCRcounts$PCRminus,PCRcounts$PCRplus1,main="counts per 1 million reads",col="red")
abline(a=c(0,0),b=(1),col="red")

plot(PCRlengths$PCRminus,PCRlengths$PCRplus1,main="repeat density",col="blue")
abline(a=c(0,0),b=(1),col="red")

plot(PCRcounts$PCRminus,PCRcounts$PCRplus2,main="counts per 1 million reads",col="red")
abline(a=c(0,0),b=(1),col="red")

plot(PCRlengths$PCRminus,PCRlengths$PCRplus2,main="repeat density",col="blue")
abline(a=c(0,0),b=(1),col="red")
...
for (threshold in 1:10) {
  ...
  #abline(v=0,col="red",lw=3)
  print(cor(subset,method="spearman"))
...
par(mfrow=c(2,3))
print(dim(subset))
#subset<-log(subset+0.0001)
#subset<-subset[3:342,]
limits<-c(-10,14000)
#limits<-c(0,10)


plot(subset$plus1,subset$plus2,xlab="PCR+, replicate 1",ylab="PCR+, replicate 2",xlim=limits,ylim=limits,main="linear relationship between PCR+")
abline(a=c(0,0),b=(1),col="red")
plot(subset$plus1,subset$minus,xlab="PCR+, replicate 1",ylab="PCR-",xlim=limits,ylim=limits,main="PCR+ counts of rep1 are overrepresented")
abline(a=c(0,0),b=(1),col="red")
plot(subset$plus2,subset$minus,xlab="PCR+, replicate 2",ylab="PCR-",xlim=limits,ylim=limits,main="PCR+ counts of rep2 are overrepresented")
abline(a=c(0,0),b=(1),col="red")
...
slope<-as.numeric(coef(res_vs_counts)[2])
plot(subset$plus1-slope*subset$plus1,subset$minus,xlim=limits,ylim=limits,main="linear relationship after correction",xlab="CORRECTED PCR+, replicate 1",ylab="PCR-")
abline(a=c(0,0),b=(1),col="red")

print(summary(lm(resid(plus1_minus)~subset$plus1)))
print(summary(lm(resid(plus2_minus)~subset$plus2)))
