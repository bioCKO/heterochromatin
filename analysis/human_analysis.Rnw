...
barplot(apply(families[2:nrow(families),c("Father","Mother","Son","Father2","Mother2","Daughter2")],2,sum),col=myColors,ylab="sum of repeat densities",main=paste("Total repeat content for",nrow(families)-1,"human repeats, excluding GGAAT"))
...
GGAAT_only<-families[1,c("Father","Mother","Son","Father2","Mother2","Daughter2")]
barplot(as.vector(as.matrix(GGAAT_only))/100,col=myColors,main="GGAAT",ylab="repeat density",names.arg=c("Father","Mother","Son","Father2","Mother2","Daughter2"))
#log_families<-log(families[,c("Father","Mother","Son","Father2","Mother2","Daughter2")]+0.0001)

smooth<-function(data_points) {
  spline<-smooth.spline(1:length(data_points), data_points/100, spar=0.05)
  return(spline)
}

lw<-3 #line width

plot(smooth(log_families$Father),type="l",xlab="repeated motif",ylab="repeat density",col=myColors[1],xlim=c(0,100),lwd=lw,main="100 most abundant repeated motifs (excluding GGAAT)")
...
plotForTrio <- function(trio_frame,family_id){
  trio_frame$Father<-(trio_frame$Father-trio_frame$Child)
  trio_frame$Mother<-(trio_frame$Mother-trio_frame$Child)
  trio_frame$Child<-(trio_frame$Child-trio_frame$Child)
  print(head(trio_frame))
  
    n=nrow(trio_frame)
    
    xmin<-min(trio_frame$Mother,trio_frame$Father)
    xmax<-max(trio_frame$Mother,trio_frame$Father)
    
    plot(head(trio_frame$Father,n=n),n:1,col="blue",pch=20,xlab="Difference from Child",ylab="Repeated motifs",yaxs="i",ylim=c(0,500),xlim=c(xmin,xmax),main=family_id) #father
    abline(v=0) 
    points(head(trio_frame$Mother,n=n),n:1,col="red",pch=20) #add mother
    segments(head(trio_frame$Father,n=n),n:1,head(trio_frame$Mother,n=n),n:1,col="gray")
  
    #text(1:n,head(r$Father,n=n),labels=head(r$V1,n=n),cex=0.4,srt=90,col="black")
    mtext(head(trio_frame$Father,n=n),cex=0.25,srt=90,col="black",side=4,at=n:1,las=1)
    legend("bottomleft", 
...
plotForTrio(family1,"family1")
plotForTrio(family2,"family2")
