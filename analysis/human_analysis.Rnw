...
plotForTrio <- function(trio_frame,family_id){ #Father #Mother #Child
  ...
  print(head(trio_frame))
    ...
    plot(head(trio_frame$Father,n=n),n:1,col=adjustcolor("blue", alpha.f = 0.5),pch=20,xlab="Difference from Child",ylab="Repeated motifs",yaxs="i",ylim=c(0,n+1),xlim=c(xmin,xmax),main=family_id) #father
    abline(v=0) 
...
myColors<-c("#2171b5","#6baed6","#bdd7e7","#d94701","#fd8d3c","#fdbe85")
family_labels<-c("father89_of_son77","mother90_of_son77","son77","father91_of_daughter78","mother92_of_daughter78","daughter78")
...
barplot(density_sums_withoutGGAAT,col=myColors,ylab="sum of repeat densities",main=paste("Total repeat content for",nrow(trios)-1,"human repeats, excluding GGAAT"),yaxt="n") 
...
stacked_density<-as.matrix(rbind(density_sums_withoutGGAAT,GGAAT_only))
barplot(stacked_density, col=c("#8dd3c7","#bebada") , border="white", space=0.04, font.axis=2, xlab="group",yaxt="n") 
axis(2, axTicks(2), format(axTicks(2), scientific = F, big.mark=",")) 
