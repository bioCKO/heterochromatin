...
print(table(apply(families[,c("Son","Father","Mother")],1,isBetweenRange)))
print(table(apply(families[,c("Daughter2","Father2","Mother2")],1,isBetweenRange)))
...
plotForTrio <- function(trio_frame,family_id){
  ...
  print(head(trio_frame))
    ...
    plot(head(trio_frame$Father,n=n),n:1,col=adjustcolor("blue", alpha.f = 0.5),pch=20,xlab="Difference from Child",ylab="Repeated motifs",yaxs="i",ylim=c(0,500),xlim=c(xmin,xmax),main=family_id) #father
    abline(v=0) 
