...
for (sp in 1:6) {
  ...
  longest<-max(c(unlist(rep_data[2:length(rep_data)]),65000),na.rm=TRUE)
  boxplot(rep_data,col=rainbow(length(repeat_types)),ylim=c(0,longest),names=motif,las=2,main=speciesType(sp),outline=TRUE)
  plot.new()
}
