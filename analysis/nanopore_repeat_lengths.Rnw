...
for (sp in c(2,3,4,5)) {
  ...
  boxplot(lapply(rep_data,log),col=rainbow(length(repeat_types)),ylim=c(0,longest),names=motif,las=2,main=speciesType(sp),outline=TRUE,yaxt='n')
...
#species_order<-c("Human","Chimp","Bonobo","Gorilla","Sorang","Borang")
species_order<-c("Human","Chimp","Gorilla","Sorang")
medians_df<-medians_df[species_order,repeat_order] #REORDER REPEAT ORDER
@
...
for (sp in c(1,2,3,4,5,6)) { 
  rownames(medians_df)[sp]
  myColors <- c("black","blue","darkgreen","red","orange","gold")
  points(1:39,medians_df[sp,1:39],col=rainbow(length(repeat_types)),las=2,main=speciesType(sp),pch=19,xaxt='n')
  lines(1:39,medians_df[sp,1:39],col=myColors[sp],las=2,main=speciesType(sp),pch=19,lw=3,xaxt='n')
  Sys.sleep(0.5)
}
axis(1, at=1:39, labels=colnames(medians_df), las=2)
@
