...
<<sanity checks>>=
trimmed<-top[,32:100]
rownames(trimmed) <- as.character(unlist(trimmed[,1]))
trimmed = trimmed[,-1 ]

sampleToCluster<-t(trimmed)
sampleToCluster<-as.data.frame(sampleToCluster)
...
sampleToCluster<-apply(sampleToCluster, 2, as.numeric)

rownames(sampleToCluster)<-make.names(groupCodes, unique=TRUE)
...
names_and_sexes<-paste(lapply(strsplit(colnames(trimmed)," "), function(x) x[1]),lapply(strsplit(colnames(trimmed)," "), function(x) x[3]))

names_of_individuals<-paste(lapply(strsplit(colnames(trimmed)," "), function(x) x[1]))
...
tr<-as.data.frame(t(trimmed[1:10,]))
tr$names<-names_of_individuals
palette(rainbow(6)) 
fit <- aov(AATGG ~ names, data=tr)
summary(fit)

#biological replicate sd
aggregate(AATGG ~ names, tr, function(x) c(mean = mean(x), sd = sd(x)))

technical<-(tr %>% group_by(names) %>% summarise_each(funs(sd),-names))
total <- tr %>% summarise_each(funs(sd),-names)

#par(mfrow=c(1,2))
technicalbox<-colMeans(technical[2:ncol(technical)])
totalbox<-colMeans(total[2:ncol(total)])
                  
#boxplot(technicalbox,col="gold",main="Technical sd",outline=FALSE,ylim=c(0,1000))
#boxplot(totalbox,col="green",main="Total sd",outline=FALSE,ylim=c(0,1000))

fun <- function(x){
    ICCbare(tr$names,x,tr)
}  
clmns<-colnames(tr)[1:ncol(tr)-1]
#boxplot(unlist(lapply(clmns,fun)),na.rm=TRUE,outline=FALSE,col="gold",main="intraclass corelation coefficient",sub=paste("n=",length(clmns)))
@
...
mycol<-colorRampPalette(c("white","black" ), bias=1) 
...
<<FIGURE3>>=
par(mfrow=c(1,5),cex=1.01, mai = c(5, 0.5, 0.3, 0.3))
...
pc <- princomp(pca_data[,1:(ncol(pca_data)-1)], cor=FALSE, scores=TRUE)
summary(pc)
...
#autoplot(prcomp(pca_data[,1:(ncol(pca_data)-1)]),colour=as.numeric(as.factor(pca_data$species)))
